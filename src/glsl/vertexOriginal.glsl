...
void main(){
    ...
    dist = clamp(0.,1.,dist*1.);

    // float xDist =  smoothstep(uHover.x, uHover.x+0.2, vUv.x)* 1.-smoothstep(uHover.x+0.3, uHover.x+0.5, vUv.x) ;
    // xDist*= uHoverState;
    float voronoi = voronoi3d(position*0.003).x;
    vDist = dist;
    newPos.x += a*sin((newPos.x*0.003+o+time*2.)*2.*PI)*scrollSpeed*10.;
    // newPos.z -= uClick*(sin(uHover.x*10.+0.25)*100.);
    newPos.z += uClick*uHoverState*dist* 50.*sin((dist*0.1+o)*2.*PI);
    // newPos.z += uClick*50.;
    ...
    // newPos.z += uHoverState*300.*voronoi;
